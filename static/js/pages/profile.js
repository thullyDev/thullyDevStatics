$(() => { 
	...
	*/
	...
	const delete_list_item = async (type, slug) => {
		const anime_loader = $(`.anime_loader[data-type="${type}"][data-slug="${slug}"]`)
	...
	const render_list = (type, data, view_type) => {
		...
		switch(view_type) {
			  case "all": 
			  	for (const key in data) {
					const item =  data[key]
					list_html += `
						<div class="other_anime_wrapper" data-slug="${item.animeId}" data-type="${type}">
							...
							item.animeTitle
							   ...
							   <button data-slug="${item.animeId}" data-type="${type}" type="button" class="delete_btn" >
								<img src="static/images/trash.svg" width="20px" height="20px" alt="delete icon" class="delete_icon">
							  </button>
	    ...
	    document.getElementById(`text_nav_view_label_wrapper`).textContent = `${type} list`
			...
			document.getElementById(`view_wrapper`).innerHTML = `
				<div id="empty_list_wrapper">
					<div id="empty_list_image_wrapper">
						<img src="/static/images/empty_list_image.png" width="20px" height="20px" alt="empty list alert image" class="empty_list_image">
					</div>
		...
		$(`.view_sort_wrapper>.view_btns_wrapper[data-type="${view_type.toLowerCase()}"]`).addClass("active_text_ele")
	...
	const get_profile_data = () => {
        ...
        success: async (res) => {
		  ...
		  const render_profile_data = (data) => {
			  console.log({data})
			  profile_image_html = data.profile_image == "None" ? `<img src="${data.default_image}" alt="account image" id="profile_account_image">${vip_logo_html}` : `<img src="${data.profile_image}" alt="account image" id="profile_account_image">${vip_logo_html}` 
			  document.getElementById("account_profile_image_wrapper").innerHTML = profile_image_html;
		  ...
		  switch(res_data.status_code) {
			  ...
			  current_page_type == "watch" ? render_list(current_page_type, temp_watch_list[0], view_type) :  render_list(current_page_type, temp_likes_list[0], view_type) 
	...
	$(".veiw_toggle_btn").click(function () {
	    ...
	    document.getElementById(`view_wrapper`).innerHTML = list_html
		$(".view_sort_wrapper>.view_btns_wrapper").removeClass("active_text_ele")
		$(`.view_sort_wrapper>.view_btns_wrapper[data-type="all"]`).addClass("active_text_ele")
	...
	$("#list_search").change(function() {
		...
		const list = current_page_type == "likes" ? Object.keys(likes_dict) : Object.keys(watch_dict)
