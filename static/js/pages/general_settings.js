$(() => {
	...
	color_inp.on("keyup input", function() {
	  ...
	  if (val.length == 3 || val.length == 4 || val.length == 6 || val.length == 7 || val.length == 8 || val.length == 9 || val.length == 10) preview_color()
	});
	...
	$(".save_btn").click(() => {
		...
		const secondary_highlight_color = $('input[data-id="secondary_highlight_color"]').val();
		...
		const highlight_accent_color = $('input[data-id="highlight_accent_color"]').val();
		...
		const recaptcha_site_key = $.trim($('input[data-id="recaptcha_site_key"]').val())
		const recaptcha_secrete_key = $.trim($('input[data-id="recaptcha_secrete_key"]').val())
		const google_verification_code = $.trim($('input[data-id="google_verification_code"]').val())
		const bing_verification_code = $.trim($('input[data-id="bing_verification_code"]').val())
		const yandex_verification_code = $.trim($('input[data-id="yandex_verification_code"]').val())
