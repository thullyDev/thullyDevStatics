$(() => {
	...
	const settings_radio_btns_wrapper = $(".settings_radio_btns_wrapper")
	const speed = 200

	
	$(".attributes_save_btn").click(() => {
		let site_attributes = {}
		
		$('input.radio_btn').each(function(){
			const this_ele = $(this)
			const id = this_ele.data("id")
			const value = this_ele.is(':checked');
			site_attributes[id] = value 
		})
		...
		$.ajax({
		  ...
		  data: {
			...
			data: JSON.stringify({site_attributes}),
		  },
		  beforeSend: () => {
	...
	$(".anime_slider_buttons").click(function() {
		...
		if (act == "delete") {
			$.ajax({
			  ...
			  success: (res) => {
				  ...
				  if(res_data.status_code == 200) {
					  ...
					  slider_length--
					  if (slider_length == 0) $(`.table_wrapper`).html(`<div class="empty_anime_slider_wrapper">No Slider Animes</div>`)
			...
			if (is_form == false) {
				 outter_anime_slider_wrapper.fadeOut(speed, () => slider_form_wrapper.fadeIn(speed))
				 is_form = true
			} else {
				slider_form_wrapper.fadeOut(speed, () => outter_anime_slider_wrapper.fadeIn(speed))
				is_form = false
			}
		}
	})
	
	$(".slider_check").change(function() {
		const this_ele = $(this)
		const value = this_ele.is(':checked');
	...
	$(".slider_btns").click(function() {
		const this_ele = $(this)
		const value = this_ele.data('value');
		const id = this_ele.data('id');
		
		$(`.slider_btns[data-id="${id}"]`).removeClass("active_btn") 
		this_ele.addClass("active_btn") 
		$(`.settings_input[data-value="${id}"]`).val(value) 
	})
	
	
	$("#add_new").click(function() {		
		settings_radio_btns_wrapper.fadeOut(speed)
		outter_anime_slider_wrapper.fadeOut(speed, () => slider_form_wrapper.fadeIn(speed))
	})
	
	$("#close_auth_icon").click(function() {
		slider_form_wrapper.fadeOut(speed, () => outter_anime_slider_wrapper.fadeIn(speed))
		settings_radio_btns_wrapper.fadeIn(speed)
	})
	
	$(".slider_save_btn").click(() => {
		...
		if (Object.keys(data).length == 8 && banner_image != "" && cover_image != "") {
			...
			$.ajax({
			  ...
			  success: (res) => {
				  ...
				  if (res_data.status_code == 200) {
					$(".cover_image").attr("src", "/static/images/default.jpg")
					$(".banner_image").attr("src", "/static/images/default.jpg")
			...
			error: function (jqXHR, textStatus, errorThrown) {
				  show_alert('images file may be to big...');
