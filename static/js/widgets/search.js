...
const render_search_results = async (val) => {
  ...
  const response = await fetch(`/search/${val}`);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    animes_html += `
        <li class="search_item_wrapper">
          <a href="/watch/${item.animeId}?gga=true" class="search_link">
            <div class="search_item_image_wrapper">
              <img src="${item.animeImg}" alt="${item.animeTitle} cover image" class="search_item_image" />
            </div>
            <div class="search_item_details_wrapper">
              <div class="search_name_des_wrapper">
                <p class="search_item_name">${item.animeTitle}</p>
              </div>
