...
const get_anime_info = async (slug, is_for_gogoanime) => {
  if (is_for_gogoanime) {
    const api_url = `https://gogoanime-thullydev-api.onrender.com/anime-details/${slug}`;
...
const wrapper_act = (wrapper_id) => {
  ...
  const wrapper_apper = () => {
    ...
    wrapper.fadeIn(fade_speed);
  };

  const wrapper_disapear = () => {
    wrapper.data("open", false);
    wrapper.fadeOut(fade_speed);
  };
      ...
      chunk.forEach((item) => {
        ...
        const top_anime_html = `
          ...
          <div class="top_anime_info_wrapper">
              ...
              <a href="/watch/${item.slug}?gga=false" class="top_anime_link">
